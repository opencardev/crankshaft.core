...
SettingsRegistry::SettingsRegistry(QObject *parent)
    : QObject(parent)
    , m_currentComplexityLevel(Basic)
{
...
void SettingsRegistry::registerCategory(const QString &id, 
                                        const QString &name, 
                                        const QString &icon,
                                        int order)
{
...
void SettingsRegistry::registerPage(const QString &categoryId,
...
void SettingsRegistry::registerSetting(const QString &pageId,
                                       const QVariantMap &setting)
{
    ...
    std::sort(m_pages[pageId].settings.begin(), 
              m_pages[pageId].settings.end(),
...
QVariantList SettingsRegistry::categories() const
{
    ...
    std::sort(sortedCategories.begin(), sortedCategories.end(),
              [](const SettingsCategory &a, const SettingsCategory &b) {
                  return a.order < b.order;
              });
    ...
    for (const auto &category : sortedCategories) {
        ...
        for (const QString &pageId : category.pageIds) {
            if (m_pages.contains(pageId) && 
                m_pages[pageId].complexityLevel <= m_currentComplexityLevel) {
...
SettingsRegistry::ComplexityLevel SettingsRegistry::complexityLevelFromString(const QString &level) const
{
