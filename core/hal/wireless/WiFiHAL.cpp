...
static const char* NM_DBUS_INTERFACE_DEVICE_WIRELESS = "org.freedesktop.NetworkManager.Device.Wireless";
...
static const char* NM_DBUS_INTERFACE_CONNECTION = "org.freedesktop.NetworkManager.Settings.Connection";
...
bool WiFiHAL::WiFiHALPrivate::findWirelessDevice() {
    ...
    for (const QDBusObjectPath& devicePath : devices) {
        QDBusInterface device(NM_DBUS_SERVICE, devicePath.path(), 
                             NM_DBUS_INTERFACE_DEVICE, dbusConnection);
...
QList<WiFiNetwork> WiFiHAL::WiFiHALPrivate::parseAccessPoints(const QList<QDBusObjectPath>& apPaths) {
...
WiFiNetwork WiFiHAL::WiFiHALPrivate::parseAccessPoint(const QString& apPath) {
    ...
    QDBusInterface ap(NM_DBUS_SERVICE, apPath, 
                     NM_DBUS_INTERFACE_ACCESS_POINT, dbusConnection);
...
WiFiHAL::WiFiHAL(QObject* parent)
    : QObject(parent)
    , d(new WiFiHALPrivate()) {
...
bool WiFiHAL::startScan() {
    ...
    QDBusInterface wireless(NM_DBUS_SERVICE, d->wirelessDevicePath,
                           NM_DBUS_INTERFACE_DEVICE_WIRELESS, d->dbusConnection);
...
bool WiFiHAL::connectToNetwork(const QString& ssid, const QString& password, WiFiSecurity security) {
    if (d->wirelessDevicePath.isEmpty()) {
        return false;
    }
    ...
    // Add connection
    QDBusPendingReply<QDBusObjectPath> addReply = settings.asyncCall("AddConnection", 
    ...
    QDBusPendingReply<QDBusObjectPath> activateReply = 
        nm.asyncCall("ActivateConnection", 
...
bool WiFiHAL::disconnect() {
    ...
    QDBusInterface device(NM_DBUS_SERVICE, d->wirelessDevicePath,
                         NM_DBUS_INTERFACE_DEVICE, d->dbusConnection);
