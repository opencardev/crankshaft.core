...
bool BluetoothHAL::BluetoothHALPrivate::findAdapter() {
  QDBusInterface manager(BLUEZ_SERVICE, "/", DBUS_OBJECT_MANAGER_INTERFACE, dbusConnection);
...
BluetoothDevice::DeviceType BluetoothHAL::BluetoothHALPrivate::guessDeviceType(
  ...
  for (const QString& uuid : uuids) {
    ...
    if (lowerUuid.contains(AUDIO_SINK_UUID) || lowerUuid.contains(HANDSFREE_UUID) ||
...
BluetoothHAL::BluetoothHAL(QObject* parent) : QObject(parent), d(new BluetoothHALPrivate()) {
  ...
  if (d->findAdapter()) {
    ...
    QDBusInterface adapter(BLUEZ_SERVICE, d->adapterPath, BLUEZ_ADAPTER_INTERFACE,
                           d->dbusConnection);
    ...
    d->dbusConnection.connect(BLUEZ_SERVICE, "/", DBUS_OBJECT_MANAGER_INTERFACE, "InterfacesAdded",
                              this,
                              SLOT(onInterfacesAdded(QDBusObjectPath, QMap<QString, QVariantMap>)));

    // Connect to InterfacesRemoved signal for removed devices
    d->dbusConnection.connect(BLUEZ_SERVICE, "/", DBUS_OBJECT_MANAGER_INTERFACE,
...
bool BluetoothHAL::setEnabled(bool enabled) {
  ...
  QDBusInterface adapter(BLUEZ_SERVICE, d->adapterPath, BLUEZ_ADAPTER_INTERFACE, d->dbusConnection);
...
bool BluetoothHAL::pairDevice(const QString& deviceAddress) {
  ...
  QDBusInterface device(BLUEZ_SERVICE, devicePath, BLUEZ_DEVICE_INTERFACE, d->dbusConnection);
