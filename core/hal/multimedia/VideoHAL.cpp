...
gboolean VideoHAL::VideoHALPrivate::busCallback(GstBus* bus, GstMessage* message, gpointer userData) {
    ...
    switch (GST_MESSAGE_TYPE(message)) {
        ...
        case GST_MESSAGE_STATE_CHANGED:
            if (GST_MESSAGE_SRC(message) == GST_OBJECT(self->d->pipeline)) {
                ...
                qDebug() << "Video pipeline state changed from" 
...
VideoHAL::VideoHAL(QObject* parent)
    : QObject(parent)
    , d(new VideoHALPrivate()) {
...
bool VideoHAL::initializePipeline() {
    ...
    gst_bin_add_many(GST_BIN(d->pipeline), d->source, d->decoder, d->convert, d->scale, d->sink, nullptr);
    ...
    g_signal_connect(d->decoder, "pad-added", G_CALLBACK(+[](GstElement* decoder, GstPad* pad, gpointer userData) {
