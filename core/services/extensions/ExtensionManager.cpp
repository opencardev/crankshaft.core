...
ExtensionManager::ExtensionManager(const QString &extensionsDir, QObject *parent)
    ...
    for (const QString &subdir : subdirs) {
        QString manifestPath = m_extensionsDir + QStringLiteral("/") + subdir
        ...
        if (manifestFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            if (!doc.isNull() && doc.isObject()) {
                ExtensionInfo info = parseManifest(doc.object(), m_extensionsDir + QStringLiteral("/") + subdir);
                if (!info.id.isEmpty()) {
...
bool ExtensionManager::validateManifest(const QJsonObject &manifest) {
    ...
    QStringList allowedPermissions{QStringLiteral("ui.tile"),
...
QString ExtensionManager::installExtension(const QString &manifestJson, const QString &targetDir) {
    ...
    QString installPath = targetDir.isEmpty() ? m_extensionsDir + QStringLiteral("/") + id
                                                : targetDir;
...
bool ExtensionManager::stopProcess(qint64 pid) {
    ...
    if (kill(pid, SIGTERM) == 0) {
        ...
        QProcess dummy;
        dummy.start(QStringLiteral("sh"), QStringList() << QStringLiteral("-c")
